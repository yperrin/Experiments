...
namespace PS
{
    ...
    public class XSLTsearch
    {
        ...
        public static string cleanSearchTerm(string data)
        {
            ArrayList patterns = new ArrayList(); //Order should be from more complex patterns to simple patterns
            patterns.Add("' \" \" '");  // double quotes in single quotes
            patterns.Add("'\" \"'");    // double quotes in single quotes
            patterns.Add("' \"\" '");   // double quotes in single quotes
            patterns.Add("'\"\"'");     // double quotes in single quotes

            patterns.Add("\" ' ' \"");  // single quotes in double quotes
            patterns.Add("\" '' \"");   // single quotes in double quotes
            patterns.Add("\"' '\"");    // single quotes in double quotes
            patterns.Add("\"''\"");     // single quotes in double quotes

            patterns.Add("\" \"");      // double quotes with a space
            patterns.Add("' '");        // single quotes with a space
            patterns.Add("\"\"");       // empty double quotes
            patterns.Add("''");         // empty single quotes

            data = System.Text.RegularExpressions.Regex.Replace(data, @"\s+", " "); //replace all kinds of whitespace with a single space
            data = data.Trim();
        ...
        public static string contextOfFind(string data, string find, int wordsBefore, int wordsAfter, int maxChars)
        {
            ...
            try
            {
                ...
                string remaining = data + " "; //v3.0.2 added space at end of data to resolve http://our.umbraco.org/projects/website-utilities/xsltsearch/xsltsearch-bugs/22868-The-very-last-word-and-previewing-it
                string output = "";
                ...
                while (output.Length < maxChars && remaining != "")
                {
                    ...
                    // 3.0.4 bug fix when preview term is last word in the search field, with no space at the end, and search term is more than maxChars from the start of the search field
                    remaining = remaining + " ";
